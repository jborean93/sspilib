...
class AscReq(enum.IntFlag):
    ...
    """
    Digest and Schannel will allocate output buffers for you. When finished,
    the buffers should be freed by calling the :meth:`free_context_buffer`
    function.
    """
...
class IscReq(enum.IntFlag):
    ...
    """
    The security package allocated the output buffers for you. When finished
    using the output buffers, free them by calling :meth:`free_context_buffer`.
    """
...
def accept_security_context(
    ...
    context_req ``ASC_REQ_ALLOCATE_MEMORY`` is specified. If
    ``ASC_REQ_ALLOCATE_MEMORY`` is used, the output buffers must be manually
    freed with :meth:`free_context_buffer` once no longer needed.
...
def initialize_security_context(
    ...
    generated by the security package. The buffers must be large enough to
    contain the data that could be generated by that buffer type or the
    context_req ``ISC_REQ_ALLOCATE_MEMORY`` is specified. If
    ``ISC_REQ_ALLOCATE_MEMORY`` is used, the output buffers must be manually
    freed with :meth:`free_context_buffer` once no longer needed.
